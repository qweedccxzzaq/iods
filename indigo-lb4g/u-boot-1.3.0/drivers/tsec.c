...
static int stub_loopback_mode = 0;
static void (*stub_loopback_rcvfunc)(volatile uchar * inpkt, int len);
static int tsec_enable_1gb_stub_loopback(struct eth_device * dev);
static int tsec_disable_1gb_stub_loopback(struct eth_device * dev);
...
/* Added by Morpheus --- Release the PHY reset pin. */
#if 1
       {
              /* Release the PHY reset pin.*/
              #include <ioports.h>
              volatile immap_t    *immap = (immap_t *)CFG_IMMR;
              volatile ioport_t *iop = ioport_addr (immap, 0);
              iop->pdat |= 0x00000010;
...
uint mii_parse_BCM54xx_psr(uint mii_reg, struct tsec_private * priv)
{
	int adv;
	int lpa;
	int lpagb = 0;

	mii_reg = read_phy_reg(priv, MIIM_CONTROL);

	if (mii_reg&MIIM_ANENABLE)
	{
		lpagb = read_phy_reg(priv, MII_STAT1000);
		adv = read_phy_reg(priv, MII_CTRL1000);
		lpagb &= adv << 2;

		lpa = read_phy_reg(priv, MIIM_TBI_ANLPBPA);
		adv = read_phy_reg(priv, MIIM_ANAR);
		lpa &= adv;
	...
	{
		if (mii_reg & MIIM_FULLDPLX)
			priv->duplexity = 1;
		else
			priv->duplexity = 0;

		if (mii_reg & MIIM_SPEED1000)
			priv->speed = 1000;
		else if (mii_reg & MIIM_SPEED100)
			priv->speed = 100;
		else
			priv->speed = 10;
	}
...
struct phy_info phy_info_BCM5482 = {
	0x0143bcb,
	"Broadcom BCM5482",
	4,
	(struct phy_cmd[]){	/* config */
	...
	(struct phy_cmd[]){	/* startup */
			   ...
			   {MIIM_STATUS, miim_read, &mii_parse_sr},
			   /* Read the status */
			   {MIIM_CONTROL, miim_read, &mii_parse_BCM54xx_psr},
			   {miim_end,}
			   },
	(struct phy_cmd[]){	/* shutdown */
			   {miim_end,}
...
#if defined(QUANTA_LB4G)
    /* Fix for reboot problem: read register twice */
    read_phy_reg(priv, MIIM_PHYIR1);
#endif
...
static int tsec_enable_1gb_stub_loopback(struct eth_device * dev)
{
    ...
    write_phy_reg(priv, 18, 0);
    rval = 0x1B00;
    write_phy_reg(priv, 9, rval);
    rval = 0x9140;
    write_phy_reg(priv, 0, rval);
    write_phy_reg(priv, 29, 7);
    rval = 0x0808;
    write_phy_reg(priv, 30, rval);
    write_phy_reg(priv, 29, 16);
    rval = 0x42;
    write_phy_reg(priv, 30, rval);
    write_phy_reg(priv, 29, 18);
    rval = 0x8901;
    write_phy_reg(priv, 30, rval);
...
int tsec_set_1gb_stub_loopback(int mode,
    void (*rcvfunc)(volatile uchar * inpkt, int len))
{
    struct eth_device * dev;
    struct tsec_private * priv;
    uint rval;
    ...
    {
        case 0:
            stub_loopback_mode = 0;
            eth_halt();
            tsec_disable_1gb_stub_loopback(dev);
            if (priv->phyinfo)
            {
                phy_run_commands(priv, priv->phyinfo->shutdown);
                udelay(100000);
        ...
        case 2:
            NetLoop(99);
            stub_loopback_rcvfunc = rcvfunc;
            stub_loopback_mode = 1;
            tsec_enable_1gb_stub_loopback(dev);
            if ( mode == 2 ) {
               priv->regs->maccfg1 |= MACCFG1_LOOPBACK; 
            }
            else if ( mode == 1 ) {
               priv->regs->maccfg1 &= ~MACCFG1_LOOPBACK; 
            }
...
int tsec_run_vct_test(uint *status)
{
    struct eth_device * dev;
    struct tsec_private * priv;
	uint result;
    ...
    /* initialize test */
	write_phy_reg(priv, 22, 1);         /* switch to page 1 */
	write_phy_reg(priv, 28, 0x0000);    /* wait for link down */
    write_phy_reg(priv, 22, 0);         /* switch to page 0 */
    write_phy_reg(priv, 28, 0x8000);    /* start VCT test */
    
    /* wait for test to complete */
    do
    {
        result = read_phy_reg(priv, 28);
    }
    while (result & 0x8000);

    /* retrieve and save test status */
    status[0] = result;
    write_phy_reg(priv, 22, 1);
    status[1] = read_phy_reg(priv, 28);
    write_phy_reg(priv, 22, 2);
    status[2] = read_phy_reg(priv, 28);
    write_phy_reg(priv, 22, 3);
    status[3] = read_phy_reg(priv, 28);
    write_phy_reg(priv, 22, 0);

    return 0;
}

int tsec_loop_rcv(struct eth_device *dev)
{
...
int tsec_mac_phy_loopback(int mode,
    void (*rcvfunc)(volatile uchar * inpkt, int len))
{
    struct eth_device * dev0, * dev1;
    struct tsec_private * priv0, * priv1;
    uint  oldrval;

    
    dev0 = eth_get_dev_by_name("TSEC0");
    dev1 = eth_get_dev_by_name("TSEC1");
    if ((dev0 == NULL)||(dev1 == NULL))
        return -1;

    priv0 = dev0->priv;
    priv1 = dev1->priv;	
    if ((priv0 == NULL)||(priv1 == NULL))
        return -1;

    switch(mode)
    {
        case 00:          /*  Disable MAC-0 Loopback test   */
            stub_loopback_mode = 0;
            eth_halt();
            priv0->regs->maccfg1 &= ~MACCFG1_LOOPBACK;     	       		
            break;
			
        case 01:          /*  Enable MAC-0 Loopback test   */
	     stub_loopback_mode = 1;	
	     stub_loopback_rcvfunc = rcvfunc;	 
            NetLoop(99);  	    	
            priv0->regs->maccfg1 |= MACCFG1_LOOPBACK;   	     	
        ...
        case 10:          /*  Disable MAC-1 Loopback test   */
            stub_loopback_mode = 0;
            eth_halt();            
	     priv1->regs->maccfg1 &= ~MACCFG1_LOOPBACK;       		
            break;

        case 11:          /*  Enable MAC-1 Loopback test   */
	     stub_loopback_mode = 1;	
	     stub_loopback_rcvfunc = rcvfunc;	 
            NetLoop(99);  	    	             
	     priv1->regs->maccfg1 |= MACCFG1_LOOPBACK;  		
	 ...
	 case 20:        /*  Disable PHY-0 Loopback test   */
		stub_loopback_mode = 0;  		              
		oldrval = read_phy_reg(priv0, 0);
	       write_phy_reg(priv0, 0, oldrval&~0x4000);				  
		phy_run_commands(priv0, priv0->phyinfo->config);
		eth_halt(); 
		break; 		
        
	 case 21:         /*  Enable PHY-0 Loopback test   */
	      stub_loopback_mode = 1;	
	      stub_loopback_rcvfunc = rcvfunc;             
	      NetLoop(99);
             priv0->regs->maccfg1 &= ~MACCFG1_LOOPBACK; 	      	 
        ...
        case 30:        /*  Disable PHY-1 Loopback test   */
		stub_loopback_mode = 0;  		              
		oldrval = read_phy_reg(priv1, 0);
	       write_phy_reg(priv1, 0, oldrval&~0x4000);				  
		phy_run_commands(priv1, priv1->phyinfo->config);
		eth_halt(); 
		break; 		
        
	 case 31:         /*  Enable PHY-1 Loopback test   */
	      stub_loopback_mode = 1;	
	      stub_loopback_rcvfunc = rcvfunc;             
	      NetLoop(99);
             priv1->regs->maccfg1 &= ~MACCFG1_LOOPBACK; 	      	 
...
void tsec_EthernetLEDInit(struct eth_device *tsecNum)
{
    struct tsec_private * tsecpriv;
    
    int    LEDREG = 0x10;
    int    SHADOWREG = 0x1c;
    int    ACTLINKvalue = 0x2400;
    int    WRITEvalue = 0x8000;
    int    LEDSELECT1 = 0x3400;
    int    LEDSELECT2 = 0x3800;
    u32  value = 0;	


    tsecpriv = tsecNum->priv;
	
      /* Set Activity/Link, and set bit 4 to 1 */	
	write_phy_reg(tsecpriv, SHADOWREG, ACTLINKvalue);	  
	value = read_phy_reg(tsecpriv, SHADOWREG);
	value |= 0x10;
	write_phy_reg(tsecpriv, SHADOWREG, value | WRITEvalue);	  
	

	/* Set Traffic Off, and set bit 5 to 0 */
	value = read_phy_reg(tsecpriv, LEDREG);	
	value &= 0xffdf;
	write_phy_reg(tsecpriv, LEDREG, value);	  		


       /* Set Link LED mode*/
	/*Link LED Mode: LINKSPD[2:1] and SLAVE LED Operation
	----------------------------------------------------------------------------------------
	LINKSPD[2]	LINKSPD[1]	SLAVE Description
	0			0			1					Force 1000BASE-T mode, no link established.
	0			1			1					Force 100BASE-TX mode, no link established.
	1			0			1					Force 10BASE-T mode, no link established.
	1			1			1					Auto-Negotiating or no link established.
	0			0			0					1000BASE-T link established.
	0			1			0					100BASE-TX link established.
	1			0			0					10BASE-T link established.
	=========================================================================================
	0 = Output driven low.
	1 = Output driven high.
	*/	

       write_phy_reg(tsecpriv, SHADOWREG, 0x8800);	  
	value = read_phy_reg(tsecpriv, SHADOWREG);
	value |= 0x1;
	write_phy_reg(tsecpriv, SHADOWREG, value | WRITEvalue);	  

		/* LINK*/	
		/* Select the shadow register */		
		write_phy_reg(tsecpriv, SHADOWREG, LEDSELECT1);	  

		/* Get the led's value from register */
		value = read_phy_reg(tsecpriv, SHADOWREG);
		value &= 0xfff0;	
		value |= 0x5;	   /*LED1 : SLAVE */

		/* Set the led's value to register */		
		write_phy_reg(tsecpriv, SHADOWREG, value | WRITEvalue );	  

		/*ACT*/	
		/* Select the shadow register */
		write_phy_reg(tsecpriv, SHADOWREG, LEDSELECT2);	  		
...
void EthernetLEDInit(void)
{
     struct eth_device * tsec0, * tsec1;


     tsec0 = eth_get_dev_by_name("TSEC0");
     tsec1 = eth_get_dev_by_name("TSEC1");	

     tsec_EthernetLEDInit(tsec0);	
     tsec_EthernetLEDInit(tsec1);	 	 
}
