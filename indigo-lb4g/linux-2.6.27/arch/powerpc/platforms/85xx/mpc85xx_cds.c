...
static void mpc85xx_cds_restart(char *cmd)
{
	...
	/*
	Use GPIO pin to reset TSEC GbE PHY, Soft Reset (CPU CPM reset) and Hard Reset (System reset).
	Modified by Raymond
	*/
	#if 1
	{
		unsigned int *gpio_membase;
		gpio_membase = ioremap((0xE0000000+0x090D10), 0x01000000);
		*gpio_membase &= ~(0x000000D0);
	}
	#endif
