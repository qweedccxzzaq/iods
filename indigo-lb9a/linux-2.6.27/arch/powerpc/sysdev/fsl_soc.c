...
/*AriesFang : 20091210*/
/*
In the MPC8548E, software can set HRESET_REQ through the reset control register (RSTCR), which is
in the reserved field of the MPC8540 or MPC8541E memory space. This feature is not supported in the
MPC8540 or MPC8541E.
The existing fsl_rstcr_restart function is not applicable to the
mpc8541. The Global Utilities Block on this earlier CPU doesn't have
the control/reset register at 0xe00b0.  This implements a board
specific reset function that uses the mpc8541's gpio to do a reset.
*/
	else
	{
		unsigned int val;
		 unsigned int immrbase;

		 printk (KERN_INFO "rstcr compatible register does not exist!\n");		
		 printk (KERN_INFO "uses the mpc8541's gpio to do a reset.\n");
		 
		immrbase = ioremap(get_immrbase(), 0x01000000);

		 val = *((unsigned int *) ((immrbase) + 0x090D10)); /*** Port A data reg ***/

	         *((unsigned int *) ((immrbase) + 0x090D10))  = val & ~(0x00000080);  /*hardware  reset*/
