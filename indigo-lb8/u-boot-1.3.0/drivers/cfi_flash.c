...
/*Yvonne turn off debug*/
#define debug_off(fmt,args...)
...
static void flash_printqry (flash_info_t * info, flash_sect_t sect)
{
	...
	for (x = 0; x < 0x40; x += 16U / info->portwidth) {
		...
		debug_off ("%p : ", cptr.cp);
		for (y = 0; y < 16; y++) {
			debug_off ("%2.2x ", cptr.cp[y]);
		}
		debug_off (" ");
		for (y = 0; y < 16; y++) {
			if (cptr.cp[y] >= 0x20 && cptr.cp[y] <= 0x7e) {
				debug_off ("%c", cptr.cp[y]);
			} else {
				debug_off (".");
			}
		}
...
#ifdef DEBUG
	debug_off ("ushort addr is at %p info->portwidth = %d\n", addr,
	       info->portwidth);
	for (x = 0; x < 2 * info->portwidth; x++) {
		debug_off ("addr[%x] = 0x%x\n", x, addr[x]);
	}
#endif
#if defined(__LITTLE_ENDIAN) || defined(CFG_WRITE_SWAPPED_DATA)
...
#ifdef DEBUG
	debug_off ("long addr is at %p info->portwidth = %d\n", addr,
	       info->portwidth);
	...
	for (sect = s_first; sect <= s_last; sect++) {
		if (info->protect[sect] == 0) { /* not protected */
			...
			case CFI_CMDSET_AMD_EXTENDED:
				...
				break;
			default:
				debug_off ("Unkown flash vendor %d\n",
				       info->vendor);
				break;
			}
...
static void flash_write_cmd (flash_info_t * info, flash_sect_t sect, uint offset, uchar cmd)
{
	...
	switch (info->portwidth) {
	case FLASH_CFI_8BIT:
		debug_off ("fwc addr %p cmd %x %x 8bit x %d bit\n", addr.cp, cmd,
		       cword.c, info->chipwidth << CFI_FLASH_SHIFT_WIDTH);
		*addr.cp = cword.c;
		break;
	case FLASH_CFI_16BIT:
		debug_off ("fwc addr %p cmd %x %4.4x 16bit x %d bit\n", addr.wp,
		       cmd, cword.w,
		       info->chipwidth << CFI_FLASH_SHIFT_WIDTH);
		*addr.wp = cword.w;
		break;
	case FLASH_CFI_32BIT:
		debug_off ("fwc addr %p cmd %x %8.8lx 32bit x %d bit\n", addr.lp,
		       cmd, cword.l,
		       info->chipwidth << CFI_FLASH_SHIFT_WIDTH);
		*addr.lp = cword.l;
		break;
	case FLASH_CFI_64BIT:
#ifdef DEBUG
		{
			...
			debug_off ("fwrite addr %p cmd %x %s 64 bit x %d bit\n",
			       addr.llp, cmd, str,
			       info->chipwidth << CFI_FLASH_SHIFT_WIDTH);
		}
#endif
...
static int flash_isequal (flash_info_t * info, flash_sect_t sect, uint offset, uchar cmd)
{
	...
	debug_off ("is= cmd %x(%c) addr %p ", cmd, cmd, cptr.cp);
	switch (info->portwidth) {
	case FLASH_CFI_8BIT:
		debug_off ("is= %x %x\n", cptr.cp[0], cword.c);
		...
		break;
	case FLASH_CFI_16BIT:
		debug_off ("is= %4.4x %4.4x\n", cptr.wp[0], cword.w);
		retval = (cptr.wp[0] == cword.w);
		break;
	case FLASH_CFI_32BIT:
		debug_off ("is= %8.8lx %8.8lx\n", cptr.lp[0], cword.l);
			...
			for (cfi_offset=0; cfi_offset < sizeof(flash_offset_cfi)/sizeof(uint); cfi_offset++) {
				...
				if (flash_isequal (info, 0, FLASH_OFFSET_CFI_RESP, 'Q')
				 ...
				 && flash_isequal (info, 0, FLASH_OFFSET_CFI_RESP + 2, 'Y')) {
					...
					debug_off ("device interface is %d\n",
						info->interface);
					debug_off ("found port %d chip %d ",
						info->portwidth, info->chipwidth);
					debug_off ("port %d bits chip %d bits\n",
...
ulong flash_get_size (ulong base, int banknum)
{
	...
	if (flash_detect_cfi (info)) {
		...
		debug_off ("manufacturer is %d\n", info->vendor);
		debug_off ("manufacturer id is 0x%x\n", info->manufacturer_id);
		debug_off ("device id is 0x%x\n", info->device_id);
		debug_off ("device id2 is 0x%x\n", info->device_id2);
		debug_off ("cfi version is 0x%04x\n", info->cfi_version);
		...
		debug_off ("size_ratio %d port %d bits chip %d bits\n",
		       size_ratio, info->portwidth << CFI_FLASH_SHIFT_WIDTH,
		       info->chipwidth << CFI_FLASH_SHIFT_WIDTH);
		debug_off ("found %d erase regions\n", num_erase_regions);
		sect_cnt = 0;
		sector = base;
		for (i = 0; i < num_erase_regions; i++) {
			...
			debug_off ("erase_region_count = %d erase_region_size = %d\n",
