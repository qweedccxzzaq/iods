...
#if defined(QUANTA_LB8) /* BinJou: 0812/2010 : LED control : LED 1 trun off  (BCM5461s) */
    uint mii_reg_led = 0;


    /* Set Activity/Link, and set bit 4 to 1 */
    write_phy_reg(priv,0x1c, 0x2400);
    mii_reg_led = read_phy_reg(priv, 0x1c);	
    mii_reg_led |= 0x10;
    write_phy_reg(priv,0x1c ,mii_reg_led | 0x8000);


    /*LINK*/
    /* Select the shadow register */	
    write_phy_reg(priv,0x1c, 0x3400);
    mii_reg_led = read_phy_reg(priv, 0x1c);
    mii_reg_led &= 0xff00;
    mii_reg_led |= 0x10;/*LED1 : LINKSPD[1] ,  LED2 : LINKSPD[2]*/
    write_phy_reg(priv,0x1c ,mii_reg_led | 0x8000);


    /* copper mode  */
    write_phy_reg(priv,0x1c, 0x7c00);
    mii_reg_led = read_phy_reg(priv, 0x1c);
    mii_reg_led &= 0xfff0;
    mii_reg_led |= 0x0;/*copper */
    write_phy_reg(priv,0x1c ,mii_reg_led | 0x8000);

    /*ACT*/
    /* Select the shadow register */
    write_phy_reg(priv,0x1c, 0x3800);
    mii_reg_led = read_phy_reg(priv, 0x1c);
    mii_reg_led &= 0xfff0;
    mii_reg_led |= 0x3;/*LED3 : ACTIVITYLED*/
    write_phy_reg(priv,0x1c ,mii_reg_led | 0x8000);
	
#else
    uint mii_reg_led = 0;
    write_phy_reg(priv,MIIM_LED_SELECT1,MIIM_LED_SELECT1_SHADOW);
	mii_reg_led = read_phy_reg(priv, MIIM_LED_SELECT1);
	mii_reg_led &= 0xfff0;
	mii_reg_led |= 0xf ;	
    write_phy_reg(priv,MIIM_LED_SELECT1 ,mii_reg_led | 0x8000);
#endif
