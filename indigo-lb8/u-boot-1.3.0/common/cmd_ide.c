...
/***AG***/
void ide_dump_data (unsigned char *s, int size ) {
        int i;
        int j;
        ...
        for (i=0;i<size;i+=16) {
            ...
            for (j=0;j<16;j++) {
                if ( (i + j) < size ) 
                    printf ("%c", (s[j] > 0x20 && s[j] < 0x7f) ? s[j] : '.' ); 
            }
            printf ("\n");
            s+=16;
        }
...
void ide_init (void)
{
#if defined(QUANTA_LB8)
       {
		...
		data = in_be32( (unsigned int*)&gur->gpindr);
              /*  Compact Flash Card Present Signal (Active Low)*/
              if (data&0x40000) /* 0000 0000 0000 0100 0000 0000 0000 0000*/
              {
                  printf ("Compact Flash Card not present!\n\n");
                  return;
              }

              /* Power Enable for Compact Flast Card.*/			  
   	       data = in_be32( (unsigned int*)&gur->gpoutdr);
	       data &= ~0x100000;  /* 0000 0000 0001 0000 0000 0000 0000 0000 */
	       out_be32( (unsigned int*)&gur->gpoutdr, data );


              /* Bus Enable for Compact Flast Card.*/
   	       data = in_be32( (unsigned int*)&gur->gpoutdr);
	       data &= ~0x10000;   /* 0000 0000 0000 0001 0000 0000 0000 0000 */
	       out_be32( (unsigned int*)&gur->gpoutdr, data );

              /* Release the Compact Flash Card reset pin.*/
   	       data = in_be32( (unsigned int*)&gur->gpoutdr);
	       data  |= 0x200000; /* 0000 0000 0010 0000 0000 0000 0000 0000 */
	       out_be32( (unsigned int*)&gur->gpoutdr, data );
       }
#endif

#ifdef CONFIG_IDE_8xx_DIRECT
__YVONNE_CHECK_THIS_SECTION_IS_NOT_COMPILED__
...
#if defined(CFG_IDE_SWAPBYTES) && defined(__PPC__)
//  The content is already big-endian encoded. NO need to swap. 
...
#if defined(CFG_IDE_SWAPBYTES) && defined(__PPC__)
//      The processor is PowerPC and the IDE drive is mapped directly
//      onto the 16-bit bus. Bytes have thus to be swapped. 
                input_swap_data (device, buffer, ATA_SECTORWORDS);
#else        
                input_data (device, buffer, ATA_SECTORWORDS*4);
#endif
                //printf ("**AG** Dumping block %d\n",blknr);
                //ide_dump_data ((unsigned char *) buffer,ATA_SECTORWORDS*4);
