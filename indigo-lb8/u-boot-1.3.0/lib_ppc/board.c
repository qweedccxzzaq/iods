...
#if defined(QUANTA_LB8) /* 10/20/2010 Yvonne. For ADT7460_1 address(2C/2D) issue.*/
  /*
    We try to power on/off LB8 for 1018 times. There are 95 times that ADT7460_1 
    read fail at first and read success in the second time.

    So we read 2C/2D 3 times here, the address should be latched successfully.
  */
  i2c_read(0x2C, 0, 0, NULL, 0);
  i2c_read(0x2D, 0, 0, NULL, 0);
  i2c_read(0x2C, 0, 0, NULL, 0);
  i2c_read(0x2D, 0, 0, NULL, 0);
  i2c_read(0x2C, 0, 0, NULL, 0);
  i2c_read(0x2D, 0, 0, NULL, 0);
...
void AdjustNetworkParam(void)
{
    ...
    {
        memcpy(&pbdinfo, dataBuffer+32, sizeof(struct SysBoardInfo));
        chksum = crc32(0, (unsigned char *)&pbdinfo, SYS_BOARD_INFO_SIZE - sizeof(unsigned int)) ;

        if (pbdinfo.checksum == chksum)
        {
            isUseDefault = 0;
            memcpy(macAddr, pbdinfo.macAddr, SYS_MAC_ADDR_SIZE);
    ...
    if (isUseDefault)
    {
        ...
        sprintf(buf, "%s", MK_STR(CONFIG_ETHADDR) );
        setenv ("ethaddr", buf);
    }
}
#endif
