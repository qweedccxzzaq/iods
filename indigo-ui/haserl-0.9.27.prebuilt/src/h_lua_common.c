...
#if 1 /* BSN: New code to avoid loading precompiled chunk */
  if (luaL_loadfile(lua_vm, HASERL_LIBRARY) != 0) {
      die_with_message (NULL, NULL, "Error loading haserl library: %s",
                        lua_tostring (lua_vm, -1));
  }

  if (lua_pcall (lua_vm, 0, 0, 0) != 0) {
      die_with_message (NULL, NULL, "Error on pcall lua vm: %s",
                        lua_tostring (lua_vm, -1));
  }
#else /* Original code */
