...
-- op=OptionParser(t)
-- op=add_option{"<opt>", action=<action>, dest=<dest>, 
--               help="<help message for this option>"}
--
-- with :
--   <opt> the option string to be used (can be anything, 
--       if one letter opt, then should be -x val, more letters: -xy=val )
--   <action> one of
...
--
-- now options is the table of options (key, val) and args is the 
-- table with non-option arguments.
-- You can use op.fail(message) for failing and op.print_help() for 
-- printing the usage as you like.
